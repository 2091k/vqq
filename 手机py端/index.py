...
def read_image_urls_from_file():
    image_urls = {}
    file_path = '/sdcard/download/py/img.txt'
    if os.path.exists(file_path):
        try:
            # 检查文件是否可读取
            if not os.access(file_path, os.R_OK):
                print("没有权限读取文件，请检查应用的存储权限。")
                return image_urls
            # 尝试以 utf-8 编码读取文件
...
def generate_html(sources):
    ...
    file_path = '/sdcard/download/py/index.html'
    try:
        # 检查文件是否可写入
        if not os.access(os.path.dirname(file_path), os.W_OK):
            print("没有权限写入文件，请检查应用的存储权限。")
            return
...
if __name__ == "__main__":
    base_dir = '/sdcard/download/py'
    sources = find_videos(base_dir)
    generate_html(sources)
    print("HTML 文件已生成：/sdcard/download/py/index.html")